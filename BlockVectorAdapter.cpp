...
/// UNTESTED: Extracts sparsity pattern from a PETScVector.
/// out param: nzentries (std::map...)
/// Returns total number of non zeros in M
std::size_t
extract_nonzeros(const PETScVector& M, nz_data_t& nzentries,
...
BlockVectorAdapter::rebuild()
{
  ...
  for (int i = 0; i < nrows; ++i)
  {
    for (int j = 0; j < ncols; ++j)
    {
      ...
      nnz += extract_nonzeros(as_type<const PETScVector>(*B), nzentries,
  ...
  // useless if I create a sequential vector here...
  ...
  _A = std::make_shared<PETScVector>(m);
}


void
BlockVectorAdapter::update(int i, int j)
{
